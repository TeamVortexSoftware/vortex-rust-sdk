...
async fn test_full_invitation_flow() {
    ...
    println!("\n--- Starting Rust SDK Integration Test ---");
    ...
    println!("Step 2a: Getting invitation by ID...");
    let invitation = public_client
        .get_invitation(&invitation_id)
        .await
        .expect("Failed to get invitation by ID");

    assert_eq!(invitation.id, invitation_id, "Retrieved invitation ID does not match expected ID");
    println!("✓ Retrieved invitation by ID successfully");

    // Step 2b: Get invitations by target
    println!("Step 2b: Getting invitations by target...");
    ...
    let found_in_list = invitations.iter().any(|inv| inv.id == invitation_id);
    assert!(found_in_list, "Invitation not found in list returned by target");
    println!("✓ Retrieved invitations by target successfully and verified invitation is in list");

    // Step 3: Accept invitation
    println!("Step 3: Accepting invitation...");
    ...
    let result = public_client
        .accept_invitations(vec![invitation_id], target)
        .await
        .expect("Failed to accept invitation");

    // The result should be an Invitation struct (even with default values for missing fields)
    println!("✓ Accepted invitation successfully (status: {})", result.status);

    println!("--- Rust SDK Integration Test Complete ---\n");
}
